...
//            .filter { !flightSearch.inTheAir || $0.departure != nil }
    ...
    private func fetchFlights() {
        ...
        let airport = flightSearch.destination
        flightAwareRequest = EnrouteRequest.create(airport: airport.icao_!, howMany: 90)
        ...
        flightAwareResultsCancellable = flightAwareRequest?.results.sink { [weak self] results in
            Airports.all.fetch(airport.icao_!) // prefetch
            results.forEach {
